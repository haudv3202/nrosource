...
public class BossChopper extends Boss {

    public BossChopper() throws Exception {
        super(BossID.CHOPPER, BossesData.BOSS_Chopper);
    }
    @Override
    public void reward(Player plKill) {
       int[] itemDos = new int[]{555, 557, 559, 556, 558, 560, 562, 564, 566, 563, 565, 567};
        int[] NRs = new int[]{17, 16};
        int [] coins = new int [] {1347,1348,1349};
        int randomDo = new Random().nextInt(itemDos.length);
        int randomNR = new Random().nextInt(NRs.length);
            int randomcoin = new Random().nextInt(coins.length);
        if (Util.isTrue(20, 100)) {
           ...
           Service.getInstance().dropItemMap(this.zone, Util.ratiItem(zone, coins[randomcoin],Util.nextInt(10, 20), this.location.x, this.location.y, plKill.id));
        }else {
...
//            if (boss.id == BossID.ZORO && boss.isDie()) {
