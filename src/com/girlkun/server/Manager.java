...
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh
    ...
    public static final String queryTopNV = "SELECT id, CAST( split_str(split_str(data_task,',',1),'[',2)  AS UNSIGNED) AS nv FROM player ORDER BY CAST( split_str(split_str(data_task,',',1),'[',2)  AS UNSIGNED) DESC, CAST(split_str(data_task,',',2)  AS UNSIGNED) DESC, CAST( split_str(data_point,',',2) AS UNSIGNED) DESC LIMIT 20;";
    public static final String queryTopSK = "SELECT id, CAST( split_str( data_inventory,',',4)  AS UNSIGNED) AS event FROM player ORDER BY CAST( split_str( data_inventory,',',4)  AS UNSIGNED) DESC LIMIT 20;";
    public static final String queryTophongngoc = "SELECT id, CAST( split_str( data_inventory,',',3)  AS UNSIGNED) AS HONGNGOC FROM player ORDER BY CAST( split_str( data_inventory,',',3)  AS UNSIGNED) DESC LIMIT 10;";
    ...
    public static final String queryTopNAP = "SELECT p.id AS id, p.account_id, a.id AS account_id, a.vnd AS nap, a.tongnap FROM player p INNER JOIN account a ON p.account_id = a.id ORDER BY nap DESC LIMIT 20;";
...
// skh tl
 ...
 public static final List<AchievementTemplate> ACHIEVEMENTS = new ArrayList<>();
    ...
    for (MapTemplate mapTemp : MAP_TEMPLATES) {
        ...
        // Create a new Thread for each map and start it
    ...
    public static void loadPart() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            dos.flush();
            dos.close();
            Logger.success("Barcoll (" + parts.size() + ")\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            Logger.success("︻┳デ═—Adu Ghê Chưa Bắn Thấy Cha Mày Bây Giờ, Nhìn Con Cặc (" + NCLASS.size() + ")\n");
            ...
            Logger.success("Barcoll(" + HEAD_AVATARS.size() + ")\n");
...
//                    dataArray.clear();
            ...
            tophongngoc = realTop(queryTophongngoc, con);
