    ...
    public boolean chuyenSinh(Player player) {
        if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 0) {
            ...
            if (player.nPoint.power >= 500000000000L) {
                ...
                hongngoc = 100000;
                //    soluong = 199;
                if ((linhhon == null || fire == null || water == null || wood == null || metal == null || tv == null) || (linhhon.quantity < 99 || fire.quantity < 99 || water.quantity < 99 || wood.quantity < 99 || metal.quantity < 99 || tv.quantity < 99)) {
                    Service.gI().sendThongBao(player, "Bạn cần thu thập đủ vật phẩm để chuyển sinh");
                } else {
                    ...
                    int kickoutChuyensinh = 5;
                    while (kickoutChuyensinh > 0) {
                        kickoutChuyensinh--;
                        Service.getInstance().sendThongBaoAllPlayer("Quá trình chuyển sinh sẽ bắt đầu trong " + kickoutChuyensinh
                                + " giây sẽ bị mất kết nối");
                        try {
                            Thread.sleep(1000);
                        } catch (Exception e) {
                        }
                    }
                    Logger.success("Player " + player.name + " chuyển sinh thành công \n");
