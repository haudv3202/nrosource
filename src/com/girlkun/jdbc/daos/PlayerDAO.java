        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = "  update player set head =?, have_tennis_space_ship = ?,"
                        ...
                        + "items_body = ?, items_bag = ?, items_box = ?, items_box_lucky_round = ?, active = ?, friends = ?,"
                        + "enemies = ?, data_intrinsic = ?, data_item_time = ?, data_task = ?, data_mabu_egg = ?, pet = ?,"
                        + "data_black_ball = ?, data_side_task = ?,data_charm = ?, skills = ?, skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?, info_achievement = ? where id = ?";
